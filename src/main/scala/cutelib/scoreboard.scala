...
case class scoreboard () extends Component with Global_parameter with Interface_MS {
  val io = new Bundle {
    ...
    val alu2_oprand_entry = master(operand_entry(CoreConfig()))  // with ALUU
    val alu3_oprand_entry = master(operand_entry(CoreConfig()))  // with ALUU
    val alu4_oprand_entry = master(operand_entry(CoreConfig()))  // with ALUU
    ...
    val alu2_ex_entry = slave(alu_res_entry(CoreConfig())) // from ex stage
    val alu3_ex_entry = slave(alu_res_entry(CoreConfig())) // from ex stage
    val alu4_ex_entry = slave(alu_res_entry(CoreConfig())) // from ex stage
    ...
    val alu2_ex_wb_entry = master(commit_entry(CoreConfig())) // to wb stage
    val alu3_ex_wb_entry = master(commit_entry(CoreConfig())) // to wb stage
    val alu4_ex_wb_entry = master(commit_entry(CoreConfig())) // to wb stage
  ...
  val rptr_real_cmt = Reg(UInt(SCB_INSTR_WIDTH+1 bit)) init(0)
  val rptr_real_cmt_next = UInt(SCB_INSTR_WIDTH+1 bit)
  ...
  //val instr_tab_full_real = (wptr(SCB_INSTR_WIDTH) ^ rptr(SCB_INSTR_WIDTH)) && (wptr(SCB_INSTR_WIDTH-1 downto 0) === rptr(SCB_INSTR_WIDTH-1 downto 0))  // 0620
  val instr_tab_full_real = (wptr(SCB_INSTR_WIDTH) ^ rptr_real_cmt(SCB_INSTR_WIDTH)) && (wptr(SCB_INSTR_WIDTH-1 downto 0) === rptr_real_cmt(SCB_INSTR_WIDTH-1 downto 0))  // 0620 for write 覆盖正在commit的指令
  val instr_tab_empty = (rptr === wptr)
  val instr_tab_empty_real = (rptr_real_cmt === wptr)
  io.scb_full := instr_tab_full_real
  val instr_end = Bool()
  //val instr_end_tab = Vec(Reg(Bool()) init(False),SCB_INSTR_DEEPTH)
  val instr_end_tab = Vec(Bool(),SCB_INSTR_DEEPTH)
  val instr_real_end_tab = Vec(Bool(),SCB_INSTR_DEEPTH)
  val ex_wb_req = Vec(Vec(Reg(Bool()) init(False),SCB_INSTR_DEEPTH) , 11)
  val wb_commit_req = Vec(Reg(Bool()) init(False),SCB_INSTR_DEEPTH)

  val ex_wb_entry_reg_wb_addr = Vec(Reg(UInt(RegAddrBus bits)) init(0) , 11)
  val ex_wb_entry_reg_wb_data = Vec(Reg(UInt(RegDataBus bits)) init(0) , 11)
  val ex_wb_entry_reg_wb_en = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_csr_wb_addr = Vec(Reg(UInt(CSRAddrBus bits)) init(0) , 11)
  val ex_wb_entry_csr_wb_data = Vec(Reg(UInt(CSRDataBus bits)) init(0) , 11)
  val ex_wb_entry_csr_wb_en = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_dcache_wb_en = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_dcache_wb_addr = Vec(Reg(UInt(DataAddrBus bits)) init(0) , 11)
  val ex_wb_entry_dcache_wb_data = Vec(Reg(UInt(DataBus bits)) init(0) , 11)
  val ex_wb_entry_dcache_rd_en = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_dcache_rd_addr = Vec(Reg(UInt(DataAddrBus bits)) init(0) , 11)
  val ex_wb_entry_dcache_rd_data = Vec(UInt(DataBus bits) , 11)
  //val ex_wb_entry_commit_req = Reg(Bool()) init(False)
  val ex_wb_entry_commit_req = Vec(Bool() , 11)
  val ex_wb_entry_instr = Vec(Reg(UInt(InstBus bits)) init(0) , 11)
  val ex_wb_entry_trans_id = Vec(Reg(UInt(SCB_ID_WIDTH bits)) init(SCB_IU_DEEPTH) , 11)
  val ex_wb_entry_dcache_wb_sel = Vec(Reg(Bits(4 bits)) init(B"1111") , 11)
  val ex_wb_entry_pc = Vec(Reg(UInt(InstAddrBus bits)) init(0) , 11)

  val ex_wb_entry_branch_cor = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_call_cor = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_ret_cor = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_target_pc = Vec(Reg(UInt(InstAddrBus bits)) init(0) , 11)

  val ex_wb_entry_is_branch = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_is_call = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_is_ret = Vec(Reg(Bool()) init(False) , 11)
  val ex_wb_entry_is_jump = Vec(Reg(Bool()) init(False) , 11)
  ...
  io.alu2_ex_wb_entry.reg_wb_addr    := ex_wb_entry_reg_wb_addr(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.reg_wb_data    := ex_wb_entry_reg_wb_data(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.reg_wb_en      := ex_wb_entry_reg_wb_en(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.csr_wb_addr    := ex_wb_entry_csr_wb_addr(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.csr_wb_data    := ex_wb_entry_csr_wb_data(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.csr_wb_en      := ex_wb_entry_csr_wb_en(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.dcache_wb_addr := ex_wb_entry_dcache_wb_addr(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.dcache_wb_data := ex_wb_entry_dcache_wb_data(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.dcache_wb_en   := ex_wb_entry_dcache_wb_en(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.dcache_wb_sel  := ex_wb_entry_dcache_wb_sel(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.commit_req     := ex_wb_entry_commit_req(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.trans_id       := ex_wb_entry_trans_id(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.dcache_rd_addr := ex_wb_entry_dcache_rd_addr(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.dcache_rd_en   := ex_wb_entry_dcache_rd_en(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.instr          := ex_wb_entry_instr(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.pc             := ex_wb_entry_pc(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.branch_cor     := ex_wb_entry_branch_cor(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.call_cor       := ex_wb_entry_call_cor(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.ret_cor        := ex_wb_entry_ret_cor(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.target_pc      := ex_wb_entry_target_pc(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.is_branch     := ex_wb_entry_is_branch(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.is_call       := ex_wb_entry_is_call(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.is_ret        := ex_wb_entry_is_ret(U(B(ALU_UNIT_SEL.ALUU2)))
  io.alu2_ex_wb_entry.is_jump        := ex_wb_entry_is_jump(U(B(ALU_UNIT_SEL.ALUU2)))

  io.alu3_ex_wb_entry.reg_wb_addr    := ex_wb_entry_reg_wb_addr(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.reg_wb_data    := ex_wb_entry_reg_wb_data(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.reg_wb_en      := ex_wb_entry_reg_wb_en(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.csr_wb_addr    := ex_wb_entry_csr_wb_addr(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.csr_wb_data    := ex_wb_entry_csr_wb_data(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.csr_wb_en      := ex_wb_entry_csr_wb_en(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.dcache_wb_addr := ex_wb_entry_dcache_wb_addr(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.dcache_wb_data := ex_wb_entry_dcache_wb_data(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.dcache_wb_en   := ex_wb_entry_dcache_wb_en(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.dcache_wb_sel  := ex_wb_entry_dcache_wb_sel(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.commit_req     := ex_wb_entry_commit_req(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.trans_id       := ex_wb_entry_trans_id(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.dcache_rd_addr := ex_wb_entry_dcache_rd_addr(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.dcache_rd_en   := ex_wb_entry_dcache_rd_en(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.instr          := ex_wb_entry_instr(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.pc             := ex_wb_entry_pc(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.branch_cor     := ex_wb_entry_branch_cor(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.call_cor       := ex_wb_entry_call_cor(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.ret_cor        := ex_wb_entry_ret_cor(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.target_pc      := ex_wb_entry_target_pc(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.is_branch     := ex_wb_entry_is_branch(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.is_call       := ex_wb_entry_is_call(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.is_ret        := ex_wb_entry_is_ret(U(B(ALU_UNIT_SEL.ALUU3)))
  io.alu3_ex_wb_entry.is_jump        := ex_wb_entry_is_jump(U(B(ALU_UNIT_SEL.ALUU3)))

  io.alu4_ex_wb_entry.reg_wb_addr    := ex_wb_entry_reg_wb_addr(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.reg_wb_data    := ex_wb_entry_reg_wb_data(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.reg_wb_en      := ex_wb_entry_reg_wb_en(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.csr_wb_addr    := ex_wb_entry_csr_wb_addr(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.csr_wb_data    := ex_wb_entry_csr_wb_data(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.csr_wb_en      := ex_wb_entry_csr_wb_en(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.dcache_wb_addr := ex_wb_entry_dcache_wb_addr(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.dcache_wb_data := ex_wb_entry_dcache_wb_data(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.dcache_wb_en   := ex_wb_entry_dcache_wb_en(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.dcache_wb_sel  := ex_wb_entry_dcache_wb_sel(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.commit_req     := ex_wb_entry_commit_req(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.trans_id       := ex_wb_entry_trans_id(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.dcache_rd_addr := ex_wb_entry_dcache_rd_addr(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.dcache_rd_en   := ex_wb_entry_dcache_rd_en(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.instr          := ex_wb_entry_instr(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.pc             := ex_wb_entry_pc(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.branch_cor     := ex_wb_entry_branch_cor(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.call_cor       := ex_wb_entry_call_cor(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.ret_cor        := ex_wb_entry_ret_cor(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.target_pc      := ex_wb_entry_target_pc(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.is_branch     := ex_wb_entry_is_branch(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.is_call       := ex_wb_entry_is_call(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.is_ret        := ex_wb_entry_is_ret(U(B(ALU_UNIT_SEL.ALUU4)))
  io.alu4_ex_wb_entry.is_jump        := ex_wb_entry_is_jump(U(B(ALU_UNIT_SEL.ALUU4)))
  ...
  ex_wb_entry_commit_req(1)   := ex_wb_req(U(B(ALU_UNIT_SEL.ALUU2 ))).orR
  ex_wb_entry_commit_req(2)   := ex_wb_req(U(B(ALU_UNIT_SEL.ALUU3 ))).orR
  ex_wb_entry_commit_req(3)   := ex_wb_req(U(B(ALU_UNIT_SEL.ALUU4 ))).orR
  ex_wb_entry_commit_req(4)   := ex_wb_req(U(B(ALU_UNIT_SEL.MULU1))).orR
  ex_wb_entry_commit_req(5)   := ex_wb_req(U(B(ALU_UNIT_SEL.MULU2))).orR
  ex_wb_entry_commit_req(6)   := ex_wb_req(U(B(ALU_UNIT_SEL.DIVU ))).orR
  ex_wb_entry_commit_req(7)   := ex_wb_req(U(B(ALU_UNIT_SEL.BJU  ))).orR
  ex_wb_entry_commit_req(8)   := ex_wb_req(U(B(ALU_UNIT_SEL.LSU  ))).orR
  ex_wb_entry_commit_req(9)   := ex_wb_req(U(B(ALU_UNIT_SEL.CSR  ))).orR
  ex_wb_entry_commit_req(10)   := ex_wb_req(U(B(ALU_UNIT_SEL.NOPU ))).orR
  ...
  ex_wb_entry_dcache_rd_data(8) :=  U"0".resized
  ex_wb_entry_dcache_rd_data(9) :=  U"0".resized
  ex_wb_entry_dcache_rd_data(10) :=  U"0".resized
  ...
  when(io.flush){
    wptr := rptr  //rptr_real_cmt + 1 //rptr + 1
  } .elsewhen(~instr_tab_full_real){
    wptr := wptr_next
  } .otherwise{}

  when(io.flush) {
    rptr := rptr  //rptr_real_cmt + 1 //rptr + 1  // rptr := rptr  // 0619
  } .elsewhen(~instr_tab_empty){
    rptr := rptr_next
  } .otherwise{}

  when(io.flush) {
    rptr_real_cmt := rptr  //rptr_real_cmt + 1  // rptr := rptr  // 0619
  } .elsewhen(~instr_tab_empty_real){
    rptr_real_cmt := rptr_real_cmt_next
  } .otherwise{}
  ...
  */
  ...
  when(instr_real_end_tab(rptr_real_cmt(SCB_INSTR_WIDTH - 1 downto 0)) === True){  // 当rptr处指令commit结束后，才会把该条指令丢弃, rptr类似sp的存在
    rptr_real_cmt_next := rptr_real_cmt + 1
  } .otherwise{
    rptr_real_cmt_next := rptr_real_cmt
  }
  ...
  val FU_ST = Vec(Reg(Bool()) init(False),11)  // True --> busy  // todo
  ...
  */
  ...
  io.alu2_oprand_entry.rs1_data := 0
  io.alu2_oprand_entry.rs2_data := 0
  io.alu2_oprand_entry.imm := 0
  io.alu2_oprand_entry.rd_addr := 0
  io.alu2_oprand_entry.rd_wten := False
  io.alu2_oprand_entry.instr := 0
  io.alu2_oprand_entry.op_type := OP_TYPE.OP_NOP
  io.alu2_oprand_entry.dec_valid := False
  io.alu2_oprand_entry.trans_id := U(SCB_IU_DEEPTH)
  io.alu2_oprand_entry.pc := 0

  io.alu3_oprand_entry.rs1_data := 0
  io.alu3_oprand_entry.rs2_data := 0
  io.alu3_oprand_entry.imm := 0
  io.alu3_oprand_entry.rd_addr := 0
  io.alu3_oprand_entry.rd_wten := False
  io.alu3_oprand_entry.instr := 0
  io.alu3_oprand_entry.op_type := OP_TYPE.OP_NOP
  io.alu3_oprand_entry.dec_valid := False
  io.alu3_oprand_entry.trans_id := U(SCB_IU_DEEPTH)
  io.alu3_oprand_entry.pc := 0

  io.alu4_oprand_entry.rs1_data := 0
  io.alu4_oprand_entry.rs2_data := 0
  io.alu4_oprand_entry.imm := 0
  io.alu4_oprand_entry.rd_addr := 0
  io.alu4_oprand_entry.rd_wten := False
  io.alu4_oprand_entry.instr := 0
  io.alu4_oprand_entry.op_type := OP_TYPE.OP_NOP
  io.alu4_oprand_entry.dec_valid := False
  io.alu4_oprand_entry.trans_id := U(SCB_IU_DEEPTH)
  io.alu4_oprand_entry.pc := 0
  ...
  for(i <- 0 until SCB_IU_DEEPTH) {
    ...
    val alu_sel_pre = ALU_SEL_TAB(index)
    val alu_sel = Reg(Bits(io.issue_dec_entry.alu_sel.getBitsWidth bits)) init(0) // 0619
    //alu_sel := alu_sel_pre
    ...
    //if (i == 0) {
    //if (U(i) == rptr) {
    val last_iu = UInt(SCB_ID_WIDTH-1 bits) // 剔除最高冗余位
    if(i==0) {
      last_iu := SCB_IU_DEEPTH-1
    } else {
      last_iu := i-1
    }
    ...
    switch(SCB_IU_TAB(i)) {
      ...
      is(IDLE) {
        ...
        //  SCB_IU_TAB(i) := IDLE
        } .elsewhen ((FU_ST(U(alu_sel_pre)) === True) && (alu_sel_pre =/= B(ALU_UNIT_SEL.ALUU))) { // Structure hazard with Function Unit // todo
          SCB_IU_TAB(i) := IDLE
        } .elsewhen(FU_ST(U(B(ALU_UNIT_SEL.ALUU))) && FU_ST(U(B(ALU_UNIT_SEL.ALUU2))) && FU_ST(U(B(ALU_UNIT_SEL.ALUU3))) && FU_ST(U(B(ALU_UNIT_SEL.ALUU4))) && (alu_sel_pre === B(ALU_UNIT_SEL.ALUU))) {
          SCB_IU_TAB(i) := IDLE
          ...
          */
          ///*
          when(alu_sel_pre=/=B(ALU_UNIT_SEL.NOPU)) {
            when(alu_sel_pre === B(ALU_UNIT_SEL.ALUU)){
              when(FU_ST(U(B(ALU_UNIT_SEL.ALUU)))){
                when(FU_ST(U(B(ALU_UNIT_SEL.ALUU2)))){
                  when(FU_ST(U(B(ALU_UNIT_SEL.ALUU3)))){
                    when(FU_ST(U(B(ALU_UNIT_SEL.ALUU4)))){
                      alu_sel := B(ALU_UNIT_SEL.ALUU)
                      FU_ST(U(B(ALU_UNIT_SEL.ALUU))) := True
                    } .otherwise{
                      alu_sel := B(ALU_UNIT_SEL.ALUU4)
                      FU_ST(U(B(ALU_UNIT_SEL.ALUU4))) := True
                    }
                  } .otherwise{
                    alu_sel := B(ALU_UNIT_SEL.ALUU3)
                    FU_ST(U(B(ALU_UNIT_SEL.ALUU3))) := True
                  }
                } .otherwise{
                  alu_sel := B(ALU_UNIT_SEL.ALUU2)
                  FU_ST(U(B(ALU_UNIT_SEL.ALUU2))) := True
                }
              } .otherwise{
                alu_sel := B(ALU_UNIT_SEL.ALUU)
                FU_ST(U(B(ALU_UNIT_SEL.ALUU))) := True
              ...
              alu_sel := alu_sel_pre
              FU_ST(U(alu_sel_pre)) := True
            }
          } .otherwise{
            alu_sel := alu_sel_pre
            FU_ST(U(alu_sel_pre)) := False  // 保证了无效指令不会阻塞
          }
          //*/
        } .otherwise {
          SCB_IU_TAB(i) := IDLE
      ...
      is(READOP) {
        ...
        when(io.flush === True) {
          ...
          switch(alu_sel) {
            ...
            is(B(ALU_UNIT_SEL.ALUU2)) {
              io.alu2_oprand_entry.rs1_data := rs1_data_real
              io.alu2_oprand_entry.rs2_data := rs2_data_real
              io.alu2_oprand_entry.imm := imm_value
              io.alu2_oprand_entry.rd_addr := rd_addr
              io.alu2_oprand_entry.rd_wten := rd_wten
              io.alu2_oprand_entry.instr := instr
              io.alu2_oprand_entry.op_type.assignFromBits(op_type)
              io.alu2_oprand_entry.dec_valid := True
              io.alu2_oprand_entry.trans_id := trans_id
              io.alu2_oprand_entry.pc := pc
            }
            is(B(ALU_UNIT_SEL.ALUU3)) {
              io.alu3_oprand_entry.rs1_data := rs1_data_real
              io.alu3_oprand_entry.rs2_data := rs2_data_real
              io.alu3_oprand_entry.imm := imm_value
              io.alu3_oprand_entry.rd_addr := rd_addr
              io.alu3_oprand_entry.rd_wten := rd_wten
              io.alu3_oprand_entry.instr := instr
              io.alu3_oprand_entry.op_type.assignFromBits(op_type)
              io.alu3_oprand_entry.dec_valid := True
              io.alu3_oprand_entry.trans_id := trans_id
              io.alu3_oprand_entry.pc := pc
            }
            is(B(ALU_UNIT_SEL.ALUU4)) {
              io.alu4_oprand_entry.rs1_data := rs1_data_real
              io.alu4_oprand_entry.rs2_data := rs2_data_real
              io.alu4_oprand_entry.imm := imm_value
              io.alu4_oprand_entry.rd_addr := rd_addr
              io.alu4_oprand_entry.rd_wten := rd_wten
              io.alu4_oprand_entry.instr := instr
              io.alu4_oprand_entry.op_type.assignFromBits(op_type)
              io.alu4_oprand_entry.dec_valid := True
              io.alu4_oprand_entry.trans_id := trans_id
              io.alu4_oprand_entry.pc := pc
            }
      ...
      is(EXE) {
        ...
        //} .elsewhen((alu_sel===B(ALU_UNIT_SEL.LSU) && io.lsu_ex_entry.load_rd_en && (io.stall_for_load || load_wait))){ // todo with read-delay && write-delay(for more cycles)
          ...
          switch(alu_sel){
            ...
            is(B(ALU_UNIT_SEL.ALUU2)){ // todo 每个ALLU_UNIT需要一组ex_wb_entry，支持并行送入WB //
              ex_wb_entry_instr(U(alu_sel)) := io.alu2_ex_entry.instr
              ex_wb_entry_pc(U(alu_sel)) := io.alu2_ex_entry.pc
              ex_wb_entry_reg_wb_en(U(alu_sel)) := rd_wten
              ex_wb_entry_reg_wb_addr(U(alu_sel)) := rd_addr
              ex_wb_entry_reg_wb_data(U(alu_sel)) := io.alu2_ex_entry.result.asUInt
              ex_wb_entry_trans_id(U(alu_sel)) := io.alu2_ex_entry.trans_id
              //ex_wb_entry_alusel(U(alu_sel)) := B(ALU_UNIT_SEL.ALUU)
              ex_wb_entry_dcache_wb_en(U(alu_sel)) := False
              ex_wb_entry_dcache_wb_addr(U(alu_sel)) := U"0".resized
              ex_wb_entry_dcache_wb_data(U(alu_sel)) := U"0".resized
            ...
            is(B(ALU_UNIT_SEL.ALUU3)){ // todo 每个ALLU_UNIT需要一组ex_wb_entry，支持并行送入WB //
              ex_wb_entry_instr(U(alu_sel)) := io.alu3_ex_entry.instr
              ex_wb_entry_pc(U(alu_sel)) := io.alu3_ex_entry.pc
              ex_wb_entry_reg_wb_en(U(alu_sel)) := rd_wten
              ex_wb_entry_reg_wb_addr(U(alu_sel)) := rd_addr
              ex_wb_entry_reg_wb_data(U(alu_sel)) := io.alu3_ex_entry.result.asUInt
              ex_wb_entry_trans_id(U(alu_sel)) := io.alu3_ex_entry.trans_id
              //ex_wb_entry_alusel(U(alu_sel)) := B(ALU_UNIT_SEL.ALUU)
              ex_wb_entry_dcache_wb_en(U(alu_sel)) := False
              ex_wb_entry_dcache_wb_addr(U(alu_sel)) := U"0".resized
              ex_wb_entry_dcache_wb_data(U(alu_sel)) := U"0".resized
            ...
            is(B(ALU_UNIT_SEL.ALUU4)){ // todo 每个ALLU_UNIT需要一组ex_wb_entry，支持并行送入WB //
              ex_wb_entry_instr(U(alu_sel)) := io.alu4_ex_entry.instr
              ex_wb_entry_pc(U(alu_sel)) := io.alu4_ex_entry.pc
              ex_wb_entry_reg_wb_en(U(alu_sel)) := rd_wten
              ex_wb_entry_reg_wb_addr(U(alu_sel)) := rd_addr
              ex_wb_entry_reg_wb_data(U(alu_sel)) := io.alu4_ex_entry.result.asUInt
              ex_wb_entry_trans_id(U(alu_sel)) := io.alu4_ex_entry.trans_id
              //ex_wb_entry_alusel(U(alu_sel)) := B(ALU_UNIT_SEL.ALUU)
              ex_wb_entry_dcache_wb_en(U(alu_sel)) := False
              ex_wb_entry_dcache_wb_addr(U(alu_sel)) := U"0".resized
              ex_wb_entry_dcache_wb_data(U(alu_sel)) := U"0".resized
      ...
      is(WB){
        ...
        //} .elsewhen(((i === rptr_real) || (i=/= rptr_real && last_iu_state === IDLE))) {  // 0620 // 上一条没有完全commit就被当前cmt指令覆盖
        } .elsewhen((last_iu_state === IDLE) || (last_iu_state === COMMIT && instr_real_end_tab(last_iu))) {  // 0621
          SCB_IU_TAB(i) := COMMIT
          instr_end_tab(i) := True  // 0620
          DEC_VLD(i) := False // 0620
          ex_wb_req(U(alu_sel))(i) := False
          ...
          SCB_IU_TAB(i) := WB
          //ex_wb_req(U(alu_sel))(i) := False // todo: 保证回写的顺序  // 0619
        }
      }
      is(COMMIT) {
        ...
        when (~(io.stall_for_commit === True && (alu_sel===B(ALU_UNIT_SEL.LSU)) && io.commit_dcache_wten === True) && (last_iu_state =/= COMMIT)) { // 0620
          //SCB_IU_TAB(i) := IDLE
          //DEC_VLD(i) := False // add for issue old instr // 0620
          //instr_end_tab(i) := True  // 0620
          instr_real_end_tab(i) := True
            wb_commit_req(i) := False
        //} .elsewhen(instr_end_tab(i) === True && ~(io.stall_for_commit === True && (alu_sel===B(ALU_UNIT_SEL.LSU)) && io.commit_dcache_wten === True)){ // 0619
          SCB_IU_TAB(i) := IDLE
          REG_ST_R(rs1_addr) := False
          REG_ST_R(rs2_addr) := False
          //REG_ST_W(rd_addr) := False
