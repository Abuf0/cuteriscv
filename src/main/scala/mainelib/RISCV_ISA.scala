...
object RISCV_ISA {
  ...
  object CSR{
    ...
    def MSTATUS (debug : Boolean) = if(debug) 0x4 else 0x300 // MRW Machine status register.
    ...
    def MTVEC     (debug : Boolean) = if(debug) 0x0 else 0x305 // MRW Machine trap-handler base address. Machine Trap Handling
    def MSCRATCH  = 0x340 // MRW Scratch register for machine trap handlers.
    def MEPC      (debug : Boolean) = if(debug) 0x1 else 0x341 // MRW Machine exception program counter.
    def MCAUSE    (debug : Boolean) = if(debug) 0x2 else 0x342 // MRW Machine trap cause.
    def MBADADDR  = 0x343 // MRW Machine bad address.
    def MTVAL     (debug : Boolean) = if(debug) 0x3 else 0x343 // the same as MBADADDR
