...
/** 暂时支支持xlen=32 **/
case class lsu_unit_queue(cfg : CoreConfig, id: Int, latency: Int = 1) extends Component {
  val io = new Bundle {
  ...
  load_rden_real := load_rden && ((io.toload_hit & U(load_byte)) =/= U(load_byte))
  ...
  val rs1_data = io.ex_in_entry(issue_id).rs1_scb_entry.reg_rdata
  val rs2_data = io.ex_in_entry(issue_id).rs2_scb_entry.reg_rdata
  ...
  */
  ...
  io.ex_out_entry.result_vld := result_vld

  when(unit_hit) {
    when(load_rden) {
      when((load_rden_real & io.mem_read_interface.rvalid) || (io.toload_hit & U(load_byte)) === U(load_byte)) {
        result_vld := True
      }
    } .otherwise{
      result_vld := True
  ...
  io.mem_read_interface.raddr := load_raddr(cfg.DataAddrBus-1 downto cfg.DMemOffset).resized//load_raddr_align
  ...
  when(unit_hit) {
    ...
    switch(io.ex_in_entry(issue_id).instr) { // latch
      is(LB) {
